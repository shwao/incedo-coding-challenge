	...
	server.get("/", {
		schema: {
			...
			querystring: Type.Object({
				...
				csvFileName: Type.String({ minLength: 1 }),
			}),
		},
	}, async (request, reply) =>
	{
		...
		try
		{
			server.csvFileSystem.writeFile(csvFileName, convertArtistsToCsv(artists));
		}
		catch (error)
		{
			return reply.code(500).send({
				error: "Internal Server Error",
				message: "Error while writing CSV file",
			});
		}

		return reply.send({ message: "File successfully written" });
	});
}